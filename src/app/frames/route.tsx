...
const handleRequest = frames(async (ctx) => {
  ...
  if (ctx.pressedButton && ctx.searchParams.type) {
    ...
    const response = await fetch(`${base.toString().replace('/frames', '/api')}/generate-story?value=${type}`);
    const data = (await response.json());
    //ctx.state = {...ctx.state, story: data.story, prompts: data.prompts };
    ctx.state.story = data.story;
    ctx.state.prompts = data.prompts;
    //ctx.state = {...ctx.state, story: data.story, prompts: data.prompts };
  }

  const stateKBs = (new TextEncoder().encode(JSON.stringify(ctx.state)).length) / 1024;
  if (ctx.state.story) {
    ...
    //if(!(ctx.state.values && ctx.state.prompts)) {
